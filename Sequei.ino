//Arduino Trigger Sequencer bases on Mutable Instruments Anushri and Grids by Thopa 2018 djthopa@djthopa.net
//
// 4 trigger Channels synced to Analog Clock
...
//Bass drum density pot  ---> A0
//Snare drum density pot ---> A1
//HiHat density pot      ---> A2
//OhHat density pot      ---> A3

//X coordinate           ---> A4
//Y coordinate           ---> A5

//Control Voltage over X coordinate          ---> A6
//Control Voltage over Y coordinate          ---> A7
// TO DO :

// Add cv control over Density & X - Y Coordinates
// Reset seems to be not working properly
...
const int rstClkIn = 3;       // The Reset Clock Input
...
const int BDrum_Out = 4;
const int SDrum_Out = 5;
const int HHats_out = 6;
const int OHats_out = 7;
...
void loop() {
  ...
  if (clkState == HIGH) {
    ...
    BDfreq = analogRead(A0);                              // read bass drum density pot
    BDfreq = map(BDfreq, 0, 1023, 255, 0);                // remap it to values similar to those in cluster configurations
    ...
    HHfreq = analogRead(A2);                              // read hi hat density pot
    HHfreq = map(HHfreq, 0, 1023, 255, 0);                // remap it to values similar to those in cluster configurations
    ...
    Xmap = analogRead(A4) + ((analogRead(A6) >> 2));     //Added External Cv control
    Serial.print("Xmap");
    Serial.print(Xmap);
    Xmap = map(Xmap, 0, 1023, -100, 100);                 // remap around zero

    // read the Y-pot for cluster navigation
    Ymap = analogRead(A5) + ((analogRead(A7) >> 2));       //Added External Cv control
    Serial.print("  Ymap");
    Serial.println(Ymap);
    ...
    if (BDrum > BDfreq) digitalWrite(BDrum_Out, HIGH);


    if (SDrum > SDfreq) digitalWrite(SDrum_Out, HIGH);


    if (HHats > HHfreq) digitalWrite(HHats_out, HIGH);


    if (OHats > OHfreq) digitalWrite(OHats_out, HIGH);
